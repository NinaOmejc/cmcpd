...
% plot lines
% ls = plot(varargin{:});
ls = plot(varargin{1}, varargin{3}, varargin{2}, 'k', varargin{5}, varargin{4},  'LineStyle', '--', 'color', '#A9A9A9');
ls(2).LineWidth = 2;
ls(2).Color = 'b';
drawnow;
hold(ax,'on');
...
for i = 1:nf
    ...
    for j = 0:length(x0)-2
        ...
        % fill polygon
        ps(i) = fill(ax,yv,xv,fc(i,:),'LineStyle','none','FaceAlpha',fa(i));

% set_amica_params() - set params for AMICA -- adaptive ICA using multiple models with shared components
%
% Usage:
%         >> set_amica_params(dat,'Key1',Value1,...);
%
% Inputs:
%
...
function [weights,sphere,mods] = set_amica_params(dat, varargin)
% if ispc
...
%     disp(['AMBIN 2:  ' AMBIN])
% else
%     AMBIN = [pwd filesep 'src' filesep 'AMICA1.7' filesep binfile];
%     disp(['AMBIN 3:  ' AMBIN])
% end
% if ismac
...
pcakeep = -1;
   ...
   for i = 1:2:length(varargin) % for each Keyword
      ...
      elseif strcmp(Keyword,'path_param_file')
         if ~isstr(Value)
            fprintf('runamica(): path_param_file must be a string');
            return
         else
            path_param_file = Value;
         end
      ...
      elseif strcmp(Keyword,'num_frames')
         if isstr(Value)
            fprintf('runamica(): num_frames must be a positive integer');
            return
         else
            frames = Value;
         end   
...
fid = fopen([path_param_file '.param'], 'w');

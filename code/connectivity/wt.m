...
function [WT,freq,mx,mxB,mxS,mxA,varargout] = wt_nina(signal,fs, varargin)
...
if nargin>2 && isstruct(varargin{1})
    ...
    if isfield(copt,'event_tbl'), cvv=copt.event_tbl; if ~isempty(cvv), event_tbl=cvv; end, end
    if isfield(copt,'fig_title'), cvv=copt.fig_title; if ~isempty(cvv), fig_title=cvv; end, end
    if isfield(copt,'visible'), cvv=copt.visible; if ~isempty(cvv), visible=cvv; end, end
    if isfield(copt,'wp')
        recflag=0;
...
for vn=vst:2:nargin-2
    ...
    elseif strcmpi(varargin{vn},'event_tbl'), if ~isempty(varargin{vn+1}), event_tbl=varargin{vn+1}; end
    elseif strcmpi(varargin{vn},'fig_title'), if ~isempty(varargin{vn+1}), fig_title=varargin{vn+1}; end
    elseif strcmpi(varargin{vn},'visible'), if ~isempty(varargin{vn+1}), visible=varargin{vn+1}; end
...
if ~strcmpi(PlotMode,'off')
    scrsz=get(0,'ScreenSize'); figure('Position',[scrsz(3)/4,scrsz(4)/8,scrsz(3)/2,6*scrsz(4)/8], 'Visible','off');
    ...
    if ~isempty(strfind(lower(PlotMode),'pow+tm'))
       ...
       axes('Position',[0.1,0.15,0.6,0.53],'Layer','top','YScale','log','Box','on','FontSize',16);
        ...
        axis tight
                        ...
                        if ~isempty(event_tbl)
                            for i = 1:size(event_tbl, 1)
                                ...
                                l = line([latency latency], [ax(3) ax(4)]);
                                set(l,'color','black','linewidth',sirnina_crt,'linestyle','--');
                                % t = text(latency, max(ylim), event_tbl.type_new{i}, 'VerticalAlignment', 'bottom', 'HorizontalAlignment', 'center');
                                % set(t,'color', 'k')
                            end
                        end
        ...
        if ~isempty(event_tbl)
            for i = 1:size(event_tbl, 1)
                latency = cell2mat(event_tbl.latency(i));
